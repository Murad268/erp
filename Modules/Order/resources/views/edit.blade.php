                            ...
                            <div class="grid grid-cols-1 gap-x-5 sm:grid-cols-2">
                                    ...
                                    <select id="products" name="products[]" multiple class="products form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 select2">
                                        @foreach($products as $product)
                                            <option value="{{ $product->id }}" {{ in_array($product->id, $order->products->pluck('id')->toArray()) ? 'selected' : '' }}>
                                                {{ $product->title }} / {{$product->price}} AZN
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('products')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select id="order_status" name="order_status" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 select2">
                                        <option value="1" {{ old('order_status', $order->order_status) == 1 ? 'selected' : '' }}>gözləmədə</option>
                                        <option value="2" {{ old('order_status', $order->order_status) == 2 ? 'selected' : '' }}>yerinə yetirilir</option>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" id="customer_surname" name="customer_surname" value="{{ old('customer_surname', $order->customer_surname) }}" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
